  ...
  <head>
    ...
    <script type="text/javascript">
       var modle="no-scanning";
        var spart_info = "";
        function save(){
            var iodepotReturner = $("#iodepotReturner").val().replace(/(^\s*)|(\s*$)/g, "");
            if(iodepotReturner == null || iodepotReturner == "" || iodepotReturner.length == 0){
                 $.messager.alert('提示', "入库人不能为空");
                 return false;
            }
            $.ajax({
				type : "post",
				url : "spart_inStock.action",
				data : "spartIds=" + spart_info + "&iodepotReturner=" + $("#iodepotReturner").val() + "&iodepotOperator=" + $("#iodepotOperator").val(),
				success: function(msg){
					$.messager.alert('入库', msg);
				}
			});
        }
	           ...
	           if(modle == "scanning"){
	               ...
	               if(sparepartNo != null && sparepartNo != "" && sparepartNo.length > 0){
		               $.ajax({
						   ...
						   data : "modle=in" + "&typeId=" + $("#typeId").val() + "&sparepartNo=" + $("#scan-box").val(),
						   success: function(msg){
							   if(msg == "no-spart"){
							       $("#scan-box").val("");
							       $.messager.alert('提示', "无此备品编号或备品已经入库");
							   }else{
							       $("#scan-box").val("");
						           var ms = msg.split(";");
     ...
     <div id="loading"></div>
  	 <form method="post" style="padding: 10px; ">
  	     <div style="margin-bottom: 10px;">
  	         ...
  	         <label style="width: 80px; font-size: 16px;">入库人：</label>
  	         <input type="text" name="iodepotReturner" id="iodepotReturner" style="width: 30%; height: 25px; background: #FFF; font-size: 17px;"/>
  	 ...
  	 <input type="text" id="scan-box" onkeyup="instock(event);" style="height:25px;">
  	 <div style="height:390px; margin-top:10px;">
	  	 <table id="browser">
	  	 	 <caption>入库清单</caption>
	  	 	 <thead>
			     <tr>
  	 ...
  	 <div id="move" style="position:absolute; width:500px; height:350px; padding:0px; border:1px solid #999; left:50px; top:100px; background:#FFF; display:none;">
  	      <div style="width:100%; height: 35px; padding:0px; background:#9CF; cursor:move;" onmousedown="mdown(event);">
  	          <label style="font-size: 22px; margin-top:0px; display: inline;">备品入库</label>
